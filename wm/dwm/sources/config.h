...
static const int ulinetop =
    0; /* 1 to show the underline above the tags, rather than under */
...
static const int refreshrate_dragcfact =
    60; /* refresh rate (per second) for dragcfact */
...
static const Layout layouts[] = {
    ...
    {"-M-",
     ...
     { -1,
       -1,
       SPLIT_CENTERED_HORIZONTAL,
       TOP_TO_BOTTOM,
       LEFT_TO_RIGHT,
       LEFT_TO_RIGHT,
       NULL }}, // centeredmaster horiz
    {":::",
     flextile,
     { -1,
       -1,
...
static const char *stoppicom[] = {"systemctl", "--user", "stop",
                                  "picom.service", NULL};
static const char *startpicom[] = {"systemctl", "--user", "start",
                                   "picom.service", NULL};

static void set_monocle_nopicom(const Arg *arg)
{
    setlayout(arg);
    if (picom_running)
    {
...
static void set_tile_picom(const Arg *arg)
{
    setlayout(arg);
    if (!picom_running)
    {
...
//{ MODKEY,                       XK_Return,     zoom,                   {0} },
    ...
    {MODKEY | Mod1Mask,
     XK_j,
     focusstack,
     {.i = +2}},                                      // The +/-2 allows focusstack to also focus on hidden
