...
enum { ClientFields, ClientTags, ClientLast }; /* dwm client atoms */
...
#if !ZOOMSWAP_PATCH || TAGINTOSTACK_ALLMASTER_PATCH ||                         \
    TAGINTOSTACK_ONEMASTER_PATCH
static void pop(Client *c);
#endif // !ZOOMSWAP_PATCH / TAGINTOSTACK_ALLMASTER_PATCH /
       // TAGINTOSTACK_ONEMASTER_PATCH
...
static int unmanaged =
    0; /* whether the window manager should manage the new window or not */
#endif // UNMANAGED_PATCH
...
static int ignore_warp =
    0; // force skip warp in some situations, e.g. dragmfact, dragcfact
#endif // WARP_PATCH
...
struct NumTags {
  char limitexceeded[NUMTAGS > 30 ? -1 : 1];
};
#else
struct NumTags {
  char limitexceeded[NUMTAGS > 31 ? -1 : 1];
};
...
/* rule matching */
  ...
  else if (c->tags != SCRATCHPAD_MASK && c->mon->tagset[c->mon->seltags])
    ...
    if (ev->window == bar->win) {
      for (r = 0; r < LENGTH(barrules); r++) {
        ...
        if (br->bar != bar->idx || (br->monitor == 'A' && m != selmon) ||
            br->clickfunc == NULL)
          continue;
        if (br->monitor != 'A' && br->monitor != -1 &&
            br->monitor != bar->mon->num)
          continue;
...
/* init layouts */
    ...
    for (j = 0; j < LENGTH(monrules); j++) {
      ...
      if ((mr->monitor == -1 || mr->monitor == m->num) &&
        ...
        m->pertag->ltaxis[i][MASTER] =
            m->pertag->ltidxs[i][0]->preset.masteraxis;
        m->pertag->ltaxis[i][STACK] =
            m->pertag->ltidxs[i][0]->preset.stack1axis;
        m->pertag->ltaxis[i][STACK2] =
            m->pertag->ltidxs[i][0]->preset.stack2axis;
#endif // FLEXTILE_DELUXE_LAYOUT
        break;
      }
    }
...
void drawbarwin(Bar *bar) {
  ...
  int rx, lx, rw,
      lw; // bar size, split between left and right if a center module is added
  const BarRule *br;
  ...
  if (bar->borderpx) {
    ...
    XSetForeground(drw->dpy, drw->gc,
                   scheme[bar->borderscheme][ColBorder].pixel);
  ...
  for (r = 0; r < LENGTH(barrules); r++) {
    ...
    if (lw <=
        0) { // if left is exhausted then switch to right side, and vice versa
      lw = rw;
      lx = rx;
    } else if (rw <= 0) {
      rw = lw;
...
int skip_bar_rule(Bar *bar, const BarRule *br) {
  Monitor *m = selmon;
...
#if NODMENU_PATCH
    XSetInputFocus(dpy,
                   selmon->bar && selmon->bar->win ? selmon->bar->win : root,
...
void focusstack(const Arg *arg) {
  ...
  if (arg->i > 0) {
    for (c = selmon->sel->next;
         c && (!ISVISIBLE(c) || (arg->i == 1 && HIDDEN(c))); c = c->next)
      ;
    if (!c)
...
Atom getatomprop(Client *c, Atom prop, Atom req) {
  int format;
      ...
      for (k = start; k <= end; k++)
        for (i = 0; i < LENGTH(on_empty_keys); i++)
          ...
          if (on_empty_keys[i].keysym == syms[(k - start) * skip])
            for (j = 0; j < LENGTH(modifiers); j++)
              XGrabKey(dpy, k, on_empty_keys[i].mod | modifiers[j], root, True,
                       GrabModeAsync, GrabModeAsync);
#endif // ON_EMPTY_KEYS_PATCH
    XFree(syms);
  }
...
/* geometry */
  ...
  if (XGetTransientForHint(dpy, w, &trans) && (t = wintoclient(trans))) {
    ...
    if (c->x == c->mon->wx && c->y == c->mon->wy)
      c->iscentered = 1;
#endif // CENTER_TRANSIENT_WINDOWS_PATCH |
       // CENTER_TRANSIENT_WINDOWS_BY_PARENT_PATCH | CENTER_PATCH
  } else {
#if SEAMLESS_RESTART_PATCH
...
#elif ALWAYSCENTER_PATCH && SAVEFLOATS_PATCH ||                                \
    ALWAYSCENTER_PATCH && EXRESIZE_PATCH
  c->sfx = c->x = c->mon->wx + (c->mon->ww - WIDTH(c)) / 2;
  c->sfy = c->y = c->mon->wy + (c->mon->wh - HEIGHT(c)) / 2;
  ...
  if (!(term && swallow(term, c))) {
    ...
    if (riopid && (!riodraw_matchpid || isdescprocess(riopid, c->pid))) {
      if (riodimensions[3] != -1)
        rioposition(c, riodimensions[0], riodimensions[1], riodimensions[2],
                    riodimensions[3]);
      else {
        killclient(&((Arg){.v = c}));
        return;
...
void resizemouse(const Arg *arg) {
  ...
  do {
    ...
    case MotionNotify:
      ...
      nw = MAX(horizcorner ? (ocw - nx) : (ev.xmotion.x - ocx - 2 * c->bw + 1),
               1);
      nh = MAX(vertcorner ? (och - ny) : (ev.xmotion.y - ocy - 2 * c->bw + 1),
               1);
#else
...
void restack(Monitor *m) {
  ...
  if (m->lt[m->sellt]->arrange) {
    ...
    if (m->bar) {
      ...
      for (f = m->stack; f && (f->isfloating || !ISVISIBLE(f)); f = f->snext)
        ; // find first tiled stack client
      if (f)
        wc.sibling = f->win;
    }
      ...
      (
          ...
          (m->lt[m->sellt]->arrange != &monocle &&
           !(m->ltaxis[MASTER] == MONOCLE &&
             (abs(m->ltaxis[LAYOUT] == NO_SPLIT || !m->nmaster ||
                  n <= m->nmaster))))
#elif MONOCLE_LAYOUT
          m->lt[m->sellt]->arrange != &monocle
#else
...
#if XKB_PATCH
    /* Unfortunately the xkbEventType is not constant hence it can't be part of
     * the normal event handler below */
    ...
    for (i = 0; i < num; i++) {
      ...
      if (wmclasscontains(wins[i], altbarclass, ""))
        ...
        if (wa.map_state == IsViewable || getstate(wins[i]) == IconicState)
          manage(wins[i], &wa);
#if SWALLOW_PATCH
        else if (gettextprop(wins[i], netatom[NetClientList], swin,
                             sizeof swin))
          manage(wins[i], &wa);
#endif // SWALLOW_PATCH
    }
    ...
    if (!selmon->lt[selmon->sellt]->arrange) {
      for (Client *c = selmon->clients; c; c = c->next) {
        if (!c->isfloating) {
          ...
          resize(c, selmon->mx + c->sfx, selmon->my + c->sfy, c->sfw, c->sfh,
                 False);
        }
      }
    }
...
#if FLEXTILE_DELUXE_LAYOUT
  if (selmon->lt[selmon->sellt]->preset.nmaster &&
      selmon->lt[selmon->sellt]->preset.nmaster != -1)
    selmon->nmaster = selmon->lt[selmon->sellt]->preset.nmaster;
  if (selmon->lt[selmon->sellt]->preset.nstack &&
      selmon->lt[selmon->sellt]->preset.nstack != -1)
...
#if PERTAG_PATCH
  selmon->pertag->ltaxis[selmon->pertag->curtag][LAYOUT] =
      selmon->ltaxis[LAYOUT];
  selmon->pertag->ltaxis[selmon->pertag->curtag][MASTER] =
      selmon->ltaxis[MASTER];
  selmon->pertag->ltaxis[selmon->pertag->curtag][STACK] = selmon->ltaxis[STACK];
  selmon->pertag->ltaxis[selmon->pertag->curtag][STACK2] =
      selmon->ltaxis[STACK2];
#endif // PERTAG_PATCH
#endif // FLEXTILE_DELUXE_LAYOUT
...
void showhide(Client *c) {
  ...
  if (ISVISIBLE(c)) {
    ...
    if ((c->tags & SPTAGMASK) && c->isfloating &&
        (c->x < c->mon->mx || c->x > c->mon->mx + c->mon->mw ||
...
#if SAVEFLOATS_PATCH || EXRESIZE_PATCH
    if (!c->mon->lt[c->mon->sellt]->arrange && c->sfx != -9999 &&
        !c->isfullscreen) {
    ...
    if (selmon->sel) {
      ...
      while (cwd) {
        if (*cwd == '~') { /* replace ~ with $HOME */
          if (!(pathbuf =
                    malloc(homelen + strlen(cwd)))) /* ~ counts for NULL term */
            die("fatal: could not malloc() %u bytes\n", homelen + strlen(cwd));
...
void togglebar(const Arg *arg) {
  ...
  selmon->showbar = selmon->pertag->showbars[selmon->pertag->curtag] =
      (selmon->showbar == 2 ? 1 : !selmon->showbar);
...
void toggleview(const Arg *arg) {
  ...
  for (selmon = mons; selmon; selmon = selmon->next) {
    ...
    Client *c;
    size_t j;
    for (c = nexttiled(selmon->clients), j = 0; c && j < selmon->nmaster;
         c = nexttiled(c->next), ++j)
...
void updatebars(void) {
  ...
  for (m = mons; m; m = m->next) {
    for (bar = m->bar; bar; bar = bar->next) {
      ...
      if (!bar->win) {
#if BAR_ALPHA_PATCH
        bar->win = XCreateWindow(dpy, root, bar->bx, bar->by, bar->bw, bar->bh,
...
void zoom(const Arg *arg) {
  ...
  if (c == nexttiled(c->mon->clients) &&
      !(c = c->mon->pertag->prevclient[c->mon->pertag->curtag] =
            nexttiled(c->next)))
    return;
  pop(c);
#else
