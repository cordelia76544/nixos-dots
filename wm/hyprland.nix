...
in {
  ...
  ## DankMaterialShell
  #############################################################
  programs.dank-material-shell.enable = hypr;
  ...
  wayland.windowManager.hyprland = {
    ...
    # DMS 自己 systemctl --user start hyprland-session.target
    ...
    settings = {
      ...
      layer_rule = [
        {match.namespace = "^(fcitx)"; no_anim = true;}
      ...
      bind =
        [
          ...
          # 多媒体
          (bindOpt "XF86AudioRaiseVolume" "hl.dsp.exec_cmd(\"wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+\")" {locked = true; repeating = true;})
          (bindOpt "XF86AudioLowerVolume" "hl.dsp.exec_cmd(\"wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-\")" {locked = true; repeating = true;})
          ...
          (bindOpt "XF86MonBrightnessUp" "hl.dsp.exec_cmd(\"${pkgs.brightnessctl}/bin/brightnessctl -e4 -n2 set 5%+\")" {locked = true; repeating = true;})
          (bindOpt "XF86MonBrightnessDown" "hl.dsp.exec_cmd(\"${pkgs.brightnessctl}/bin/brightnessctl -e4 -n2 set 5%-\")" {locked = true; repeating = true;})
    ...
    ###########################################################
    ## DMS 模块 + 冲突键位重绑
    ## extraConfig 追加在末尾，因此这里的 bind 优先级最高
    ###########################################################
    extraConfig = ''
      require("dms.colors")
      require("dms.outputs")
      ...
      -- dms/binds-user.lua 里有 hl.unbind("SUPER + Q")，在此重新绑定
      hl.unbind("SUPER + Q")
